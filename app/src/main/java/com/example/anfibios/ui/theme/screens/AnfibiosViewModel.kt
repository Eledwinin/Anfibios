...
sealed interface AnfibiosUiState{
    data class Success(val infoAnfibios: List<Anfibios>): AnfibiosUiState
...
class AnfibiosViewModel(private val anfibiosRepository: AnfibiosRepository): ViewModel() {
    ...
    var anfibiosState: AnfibiosUiState by mutableStateOf(AnfibiosUiState.Loading)
         private set
    ...
    fun getAnfibiosDatos(){
        viewModelScope.launch{
            anfibiosState = AnfibiosUiState.Loading
            anfibiosState = try{
                //aqui vamos a guardar la lista completa de anfibios
